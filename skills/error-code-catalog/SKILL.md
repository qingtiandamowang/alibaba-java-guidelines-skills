...
description: 阿里巴巴错误码体系与完整错误码列表：A/B/C 三段式分类（A 用户端错误、B 系统执行错误、C 第三方服务错误），含一级/二级宏观错误码。Use when designing API error codes, defining an error taxonomy, or looking up a standard error code — in ANY language, since the taxonomy is transport- and language-independent.
---

# 错误码列表

**三段式分类**：`A` 开头为用户端错误，`B` 开头为系统执行错误，`C` 开头为第三方服务错误。每类下设一级宏观错误码与二级具体错误码。

**格式**：`错误码  中文描述  说明`

**来源**：《阿里巴巴Java开发手册（黄山版）》附3。
