...
description: 单元测试规约：AIR 原则（自动化/独立性/可重复）、必须用 assert 验证而非打印、用例间禁止互相依赖、边界值测试、测试粒度与覆盖率（源自阿里巴巴Java开发手册第三章）。Use when writing unit tests or test cases, or reviewing test quality — in ANY language (Go, Rust, Python, Java), since these are testing-discipline principles independent of language.
---
...
产生的数据有明确的前后缀标识。
正例：在基础技术部的内部单元测试中，使用FOUNDATION_UNIT_TEST_ 的前缀来标识单元测试相关代码。
