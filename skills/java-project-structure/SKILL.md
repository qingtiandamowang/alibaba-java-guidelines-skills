...
description: 阿里巴巴Java开发手册工程结构规约：应用分层、分层领域模型（DO/DTO/BO/VO）、二方库依赖、服务器配置，附专有名词解释。Use ONLY when structuring a Java/Maven project — covers Maven GAV coordinates, jar dependency management, Tomcat tuning. Do NOT use for Go modules, Rust crates, or other non-Java build systems.
---
...
3.【强制】 线上应用不要依赖 SNAPSHOT 版本（安全包除外）；正式发布的类库必须先去中央仓库进行查
...
免得不同服务的线程同归于尽。
8.【参考】 服务器内部重定向必须使用 forward ；外部部重定向地址必须使用 URL Broker 生成，否则因线
...
DO / DTO / BO / VO 等。
2. DO（Data Object ）： 阿里巴巴专指数据库表一  一对应的 POJO 类。  此对象与数据库表结构一一对应，通过 DAO 层
向上传输数据源对象。
