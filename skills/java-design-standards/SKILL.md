...
description: 设计规约：需求分析与用例图、状态图、领域模型设计、系统设计文档、可扩展性与变化点隔离、无障碍设计（源自阿里巴巴Java开发手册第七章）。Use when designing software architecture, modelling a domain, or making design decisions — in ANY language, since these are design-discipline principles independent of language.
---
...
2.【强制】 在需求分析阶段，如果与系统交互的 User 超过一类并且相关的 UseC ase 超过5个，使用用例图
来表达更加清晰的结构化需求。
...
不可能有直接转换关系的。
4.【强制】 如果系统中某个功能的调用链路上的涉及对象超过 3个，使用时序图来表达并且明确各调用环
节的输入与输出。
说明：时序图反映了一系列对象间的交互与协作关系，清晰立体地反映系统的调用纵深链路。
5.【强制】 如果系统中模型类超过 5个，且存在复杂的依赖关系，使用类图来表达并且明确类之间的关系。
...
原因可能为调用的外部依赖服务 RT过高而导致系统假死，而在显示端没有做降级预案，只能直接抛错给用户。
8.【推荐】 系统架构设计时明确以下目标：
